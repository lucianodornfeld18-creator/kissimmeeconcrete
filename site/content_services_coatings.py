...
def stucco():
    ...
    body = sec("Stucco repair and installation in Kissimmee",
        cap("Hairline stucco cracks at window corners and control joints are normal on Central Florida block homes; cracks wider than 1/16 inch, bulging, hollow sounds or staining at the bottom of a wall point to water behind the stucco and need a patch or a re-lath. Check the installer's insurance and references, and get the repair scope in writing."),
        eyebrow="Quick answer")
    ...
    faqs = [faq("Are stucco cracks normal?", "Hairline cracks at corners and joints are. Wider, stepped or wet cracks are not."), faq("How do I know a stucco repair was done properly?", "The texture match should be invisible from ten feet, the patch should sound solid rather than hollow, and the repair should be primed ready for paint.")]
    return {"route": SERVICES[k]["route"], "title": "Stucco Repair & Installation in Kissimmee, FL", "meta_description": "Stucco crack repair, patching and re-stucco on block homes in Kissimmee and Osceola County: which cracks matter, texture matching, and how to tell a proper repair from a cosmetic one.", "h1": "Stucco repair and installation in Kissimmee, FL", "breadcrumbs": [("Home", "/"), ("Exterior", "/pavers/"), (SERVICES[k]["name"], None)], "body_html": body, "faqs": faqs, "kind": "service", "service_key": k, "nav_active": "/pavers/", "sources": []}
