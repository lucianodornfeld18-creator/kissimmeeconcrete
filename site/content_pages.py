...
def privacy():
    body = sec("What we collect", f"<p>When you send the estimate form we receive the name, phone, email, ZIP code, service, property type, timeline, notes and optional photo you enter, plus the page you sent it from, the referring site, any campaign tags in the URL (utm_*, gclid), a timestamp and the country of your connection. Cloudflare Turnstile checks that the sender is a person; it may set a cookie for that purpose. We do not collect anything from the calculators, the permit finder or the project brief tool: they run in your browser and the address you type into the permit finder is sent only to the U.S. Census Bureau geocoder, not to us.</p>")
    body += sec("How we use it", f"<p>To reply to you about the estimate you asked for, to schedule and perform the work, and to keep a record of the job. If we text you, standard message rates apply and you can reply STOP at any time to stop them. We do not sell your information and we do not share it with anyone other than the crew that performs your work and the service providers that deliver our email and hosting (Cloudflare).</p>", cls="alt")
